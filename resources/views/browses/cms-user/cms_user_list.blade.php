...
@section('content')
    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body">
            <a href="{{ url('browse/cms-user/add') }}" class="btn btn-success btn_add_user" style="margin-bottom: 15px;">
                ...
                <tbody id="">
                    @if (!empty($cms_users))
                        @foreach ($cms_users as $user)
                            <tr style="text-align: center; {{ $user['role'] == 'super_admin' ? 'color:red;' : '' }}">
                                <td>{{ $user['name'] }}</td>
                                <td>{{ $user['email'] }}</td>
                                ...
                                <td style="width: 90px;">
                                    <a href="{{ url('browse/cms-user') }}/{{ $user['id'] }}" class="btn btn-sm blue"><i class="fa fa-search"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    @endif
